...
public class SPUM_Exporter : MonoBehaviour
{
	...
	public void MakeScreenShotFile()
	{
		...
		for(var i = 0 ; i < resultImages.Count;i++)
		{
			...
			if(!Directory.Exists("Assets/ThirdParty/SPUM/ScreenShots/"))
			{
				Directory.CreateDirectory("Assets/ThirdParty/SPUM/ScreenShots/");
			}

			string filename = string.Format("{0}/ThirdParty/SPUM/ScreenShots/{1}_{2}.png", Application.dataPath,tName,i);
...
// for gif exporter - preview version
	void PreProcess()
	{
		...
		if(!Directory.Exists("Assets/ThirdParty/SPUM/ScreenShots/GifExports"))
		{
			Directory.CreateDirectory("Assets/ThirdParty/SPUM/ScreenShots/GifExports");
		}
		string filepath = Application.dataPath + "/ThirdParty/SPUM/ScreenShots/GifExports/" + _unitPrefab.name+"_"+_sepaName+".gif";
		for(var i = 0 ; i < _textSaveList.Count ; i++)
		{
